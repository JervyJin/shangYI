  ...
  <div class="content">
    ...
    <div class="container">
      <van-row class="user-links">
        <van-col span="7" class="mt-20 pr-10">
          <img :src="userInfo.HeadPortrait" width="100%" class="imgradius" v-if="userInfo.HeadPortrait"/>
          ...
          <van-col span="24" class="mt-20">
            ...
            <van-col span="7">
              <router-link :to="'/StickTag/'+id">
                <div class="Alabel f-12">+贴标签</div>
              </router-link>
            </van-col>
          </van-col>
          <van-col span="24" class="mt-10">
            <p class="c-blue f-12">
              <span class="display mr-5 f-l" v-for="(item, index) in userInfo.TagName" :key="index">#{{item}}#</span>
      ...
      <van-row class="row text-c mt-20 backblue">
        <van-col span="12" class="line">
          <span class="f-18">
            <i class="iconfont mr-5 f-16" slot="icon" >&#xe945;</i>{{userInfo.Score}}
          </span>
          <p>积分</p>
        </van-col>
        <van-col span="12">
          <div>
            <span class="f-18">
              <i class="iconfont mr-5 f-16" slot="icon">&#xe626;</i>{{userInfo.AccountBalance}}
            </span>
            <p>余额</p>
          </div>
        </van-col>
    ...
    <div class="container mt-0" v-if="xfInfo.CreatorTime">
      ...
      <div class="conall f-14">
        <div class="xf-item" v-for="(item, index) in xfInfo.ListGoods" :key="index">{{item.GoodName}}*{{item.Num}}</div>
      </div>
    </div>
    <!-- <div class="container mt-0">
      <van-cell-group class="user-group border-bottom">
...
<script>
  import { getMemberInfo, getExpendRecord, getExpendDetailUser } from '../../api/hy'
      ...
      getExpendRecord({
        ...
        if(res.Rows.length > 0) {
          getExpendDetailUser(res.Rows[0].Id).then(res =>{
            if(res.Data) {
              res.Data.CreatorTime = dayjs().diff(dayjs(res.Data.CreatorTime),'day')
