<template>
  <div class="container">
    ...
    >
      ...
      <GridColumn field="TemplateCode" title="短信模板编号" align="center">
        <template slot="body" slot-scope="scope">
          <div align="right">{{scope.row.TemplateCode}}</div>
        </template>
      </GridColumn>
      <GridColumn field="MessageType" title="短信模板标题" align="center">
        <template slot="body" slot-scope="scope">
          <div align="left">{{scope.row.MessageType}}</div>
        </template>
      </GridColumn>
      <GridColumn field="TemplateId" title="短信模板ID" align="center">
        <template slot="body" slot-scope="scope">
      ...
      <GridColumn field="UpdateName" title="修改人" align="center">
        <template slot="body" slot-scope="scope">
          <div align="left">{{scope.row.UpdateName}}</div>
        </template>
      </GridColumn>
      <GridColumn field="UpdateTime" title="修改时间" align="center">
        <template slot="body" slot-scope="scope">
    ...
    <Dialog
      ref="MenberAddDialog"
      :title="title"
      :dialogStyle="{width:'825px',height:'550px'}"
      bodyCls="f-column"
      :modal="true"
      :draggable="true"
      :resizable="true"
    ...
    >
      ...
      ></smsEditTemplate>
      <template v-slot:footer>
        <LinkButton @click="$refs.EditTemplate.submitForm()" class="btnConfirm">确认</LinkButton>
        <LinkButton @click="$refs.EditTemplate.closeDialog()" class="btnCancel">取消</LinkButton>
      </template>
    </Dialog>
  </div>
</template>
...
import { GetNoteTemplate, UpdateNoteTemplateState } from "@/api/Service";
import { parseTime } from "@/utils/index";
import smsTemplate from "@/components/smsTemplate";
import smsEditTemplate from "@/components/smsEditTemplate";
import {
  DeleteMarketing,
...
export default {
  ...
  components: { smsTemplate, smsEditTemplate },
  methods: {
    queryToggle() {
    ...
    disableBtn(index, row) {
      ...
      console.log(para);
      UpdateNoteTemplateState(JSON.stringify(para)).then(res => {
